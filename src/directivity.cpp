...
int load_directivity(const char* matpath, directivity_t* directivity) {
  ...
  directivity->coords = (spherical_coords_t*)malloc(sizeof(spherical_coords_t) *
                                                    directivity->n_recievers);
  for (i = 0; i < directivity->n_recievers; i++) {
    directivity->coords[i] = {
        .azimuth = azimuth[i], .elevation = M_PI_2 - colatitude[i], .r = R};
  }
  ...
  directivity->n_fft =
      matvar->nbytes / matvar->data_size / directivity->n_recievers;

  // Obtain FFT of impulse responses of each row
  fftw_in = (double*)malloc(sizeof(double) * directivity->n_fft);
  fftw_out = (std::complex<double>*)malloc(sizeof(std::complex<double>) *
                                           directivity->n_fft);
  directivity->tfs = (std::complex<double>*)malloc(
      sizeof(std::complex<double>) * directivity->n_recievers *
      directivity->n_fft);
  fftw_plan plan = fftw_plan_dft_r2c_1d(
      directivity->n_fft, fftw_in, reinterpret_cast<fftw_complex*>(fftw_out),
      FFTW_ESTIMATE);
  for (int i = 0; i < directivity->n_recievers; i++) {
    memcpy(fftw_in, directivity->irs + i * directivity->n_fft,
           directivity->n_fft);
    fftw_execute(plan);
    memcpy(directivity->tfs + i * directivity->n_fft, fftw_out,
           directivity->n_fft);
  }
